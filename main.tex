\documentclass[11pt,fontset=ubuntu,notitlepage]{ctexrep}
...
% https://tex.stackexchange.com/a/762054/322482
\cfoot{\tikz\pic[scale=.5,duck/randomhead,duck/randomaccessories,duck/signback=white,duck/signpost={\parbox{6cm}\centering\color{black}\bfseries\thepage}] {duck};}
...
\lstset{
  ...
  morekeywords    = { add-apt-repository, apt, brew, certutil, evince,
                      ...
                      visudo, wget, wsl, xelatex, lualatex },
...
\lstdefinelanguage{mwe}{
  ...
  morekeywords = {ctexart, document, LaTeX},
...
\makeatother
\title{\huge\bfseries Vscode+LaTeX Workshop+TeXLive配置指南\thanks{\url{https://github.com/Explorer-cc/vscode-latexworkshop-guide}}}
\author{\Large\bfseries Explorer\thanks{\href{mailto:Explorer\_live@yeah.net}{\texttt{Explorer\_live@yeah.net}}}}
\date{\Large \today}
...
首先相信每位读到本文的用户面前肯定已经打开了一个\texttt{.tex}文件，它可能是「2077年全国大学生数学竞赛模板」、或者是「广为流传的\href{https://github.com/ElegantLaTeX/ElegantBook}{\texttt{elegantbook}系列模板}」，如果你还没有配置好vscode的编译环境，请先\textbf{关闭这些较为复杂的模板}，从下面这个\textbf{最简单}的例子开始.

\textbf{罗马不是一天建成的，还没学走路就想会飞，只会在配置的过程中遇到更多的问题}，最终因为配置和 debug 极其麻烦而更容易打退堂鼓.
...
\begin{itemize}
  ...
  \item \textbf{编译器（compiler）}才是真正用来「把代码转换成PDF」的工具. 这里我们需要使用的编译器是\texttt{TeXLive}，它是一个非常庞大的编译器集合，里面包含了各种各样的编译器. 注意，你大致可以把编译器理解为「可执行程序（e.g. \texttt{Genshin Impact.exe}）」，这类程序执行的时，会按照其内部逻辑处理你的代码，并根据程序设定好的功能执行任务，具体到在TeXLive中的编译器，其功能正是把你的\texttt{.tex}文件按照\TeX{}的规则生成\texttt{.pdf}等目标文件.
\end{itemize}

一言以蔽之，我们在这里要实现的，是用「vscode」作为代码\textbf{编辑器}来编写代码，并调用我们目标的\textbf{编译器}（这里是\texttt{TeXLive}中的\texttt{pdflatex.exe}、\texttt{luahbtex.exe}等），通过这些程序将我们编写的代码转换成目标的PDF文件.
...
\begin{center}
  \includegraphics[width=.75\textwidth]{open-terminal.png}
\end{center}

如果找不到这两项，那么也请自行通过搜索引擎检索如何把它们调出来. 本文采用「通过Code打开」作为示例，如果你不能看到下方的「Terminal」面板，可以通过快捷键「Ctrl+\`」呼出.
...
你可能听说过需要配置一个名为\texttt{settings.json}的文件，但这是必须的吗？并不是，LaTeX Workshop贴心地提供了一份\textbf{默认}（虽然事实上现在已经并非默认，但作例子合适）的\href{https://github.com/James-Yu/LaTeX-Workshop/wiki/Compile#latex-tools}{tools}和\href{https://github.com/James-Yu/LaTeX-Workshop/wiki/Compile#latex-recipes}{recipes}配置:
...
而所谓的\textbf{配方}（recipes），我本人更喜欢将其翻译为\textbf{编译链路}），实际上只是多个tool的组合而已，同样举个例子:
\begin{lstlisting}[language=json]
  {
      "name": "pdflatex -> bibtex -> pdflatex * 2",
      "tools": [
...
这定义了一个名为「\texttt{pdflatex -> bibtex -> pdflatex * 2}」的\textbf{编译链路}，当用户选择该编译链路时，\textbf{LW}会\textbf{先后}在终端帮助你自动依次执行四个工具「\texttt{pdflatex}」-「\texttt{bibtex}」-「\texttt{pdflatex}」-「\texttt{pdflatex}」，也即分别为:
\begin{lstlisting}[language=bash]
  pdflatex -synctex=1 -interaction=nonstopmode -file-line-error <filename>
  bibtex <filename>
  pdflatex -synctex=1 -interaction=nonstopmode -file-line-error <filename>
...
打开之后我们可以看到一个可能是空白（对于第一次使用vscode的用户这个文件是一个空文件的，而对于已经使用过vscode编辑其他语言的代码的用户，此时其中往往已有一些其他语言的配置）的\texttt{settings.json}配置文件. 我们需要做的配置就是按照\texttt{json}的语法往其中填写配置.
...
\mybullet{使用「vscode+LaTeX Workshop」与「TeXStudio」方案的功能差异在哪？}
  ...
  \begin{leftbar}
    ...
    个人觉得LW不仅是能完全替代TeXStudio(TXS)，甚至超越了TXS，代码补全做的很好，连22年出的\texttt{physics2}包的自动补全都在规则内（而且是可配置的）. 因为LW开发者很活跃，bug修的很快（issues数量常年维持在个位数）. 而且可以配置一些你意想不到的编译序列（开发者用的多）.
    % ToDo.口语化
  \end{leftbar}
