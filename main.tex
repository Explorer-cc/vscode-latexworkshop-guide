...
--     --https://tex.stackexchange.com/a/760670 (Jasper Habicht answer)
--     local g = graph:new{window={-5,5,-5,5},margin={0,0,0,0},size={2,2}}
--     local i = cpx.I
...
这定义了一个名为「\texttt{pdflatex -> bibtex -> pdflatex * 2}」的\textbf{编译链路}（recipes），当你选择它时，\textbf{LW}会\textbf{先后}在终端帮助你自动依次执行四个工具「\texttt{pdflatex}」-「\texttt{bibtex}」-「\texttt{pdflatex}」-「\texttt{pdflatex}」，也即分别为:
\begin{lstlisting}[language=bash]
pdflatex -synctex=1 -interaction=nonstopmode -file-line-error <filename>
...
在这里，我想\textbf{邪恶地}留一个习题，如果能顺利做出来，那么以上的内容应该是完全掌握了.
...
\begin{itemize}
  \item \textbf{作为萌新用户在安装后如何进一步入门？}
  \begin{featurebox}
    阅读经典教程「\href{https://ctan.org/pkg/lshort-zh-cn}{lshort-zh-cn}」.
  \end{featurebox}

  \item \textbf{不同\texttt{tools}的差异以及该如何使用：}
  \begin{featurebox}
  ...
  \item \textbf{遇到编译错误「\texttt{Recipe Terminated with error.}」如何排查?}
  ...
  \item \textbf{遇到编译不成功，左下角出现「红色的 $\times$ 符号」但「Problems」面板中\textbf{没有任何报错信息}，如何排查错误原因?}
  ...
  \item \textbf{使用「vscode+LaTeX Workshop」与「TeXStudio」方案的功能差异在哪？}

  \begin{featurebox}
    此事在「\href{https://github.com/OsbertWang/install-latex-guide-zh-cn}{install-latex-guide-zh-cn}」中亦有记载：
    \begin{center}
      \includegraphics[width=0.9\textwidth]{editors.png}
    \end{center}
    ...
    个人觉得LW不仅是能完全替代TXS，甚至超越了TXS，代码补全做的很好的，连22年出的\texttt{physics2}包的自动补全都在规则内（而且是可配置的）. 因为LW开发者很活跃，bug修的很快（issues数量常年维持在个位数）. 而且可以配置一些你意想不到的编译序列（开发者用的多）.
  \end{featurebox}

  \item \textbf{对编译选项\texttt{-interaction=nonstopmode}的看法}
  ...
  \item \textbf{什么时候需要使用编译选项「\texttt{-shell-escape}」？}
  \begin{featurebox}
  ...
  \item \textbf{使用「vscode+LaTeX Workshop」如何实现自动编译？}
  \begin{featurebox}
  ...
  \item \textbf{设置遇到警告和错误时不显示「烦人的下划曲线」}
  \begin{featurebox}
  ...
  \item \textbf{设置不显示「烦人的\texttt{badbox}提示」}
  \begin{featurebox}
